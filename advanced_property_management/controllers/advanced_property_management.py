...
class PropertyController(http.Controller):
    ...
    # @http.route('/signup', auth='user', website=True)
    # def signup(self):
    #     """ Returns the property_view for the route """
    #     return request.render('advanced_property_management.signup_view', {})

    # apartment start
    @http.route('/apartment/<int:apartment_id>', auth='user', website=True)
    def apartment_item(self,apartment_id):
        """ Shows each corresponding apartment view in apartment_view_item """
    ...
    @http.route('/book/apartment', type='http',auth='user', website=True)
    def book_apartment(self):
        """ Shows  booking apartment form """
        countries = request.env['res.country'].sudo().search([])
        states= request.env['res.country.state'].sudo().search([])
        apartments =  request.env['property.apartment'].sudo().search([])
        return request.render('advanced_property_management.contact_apartment_view', {
            'countries':countries,
            'states':states,
    ...
    def create_booking_apartment(self,**kw):
        """ Shows  booking apartment form """
        print('vals',kw)
        request.env['apartment.booking'].sudo().create(kw)
        return request.render('advanced_property_management.apartment_thanks', {})
